...
- 서비스 URL: https://fanflow.hongtfolio.kr
- Backend API: https://fanflow.hongtfolio.kr/api
...
* 게시글/댓글 신고 및 관리자 신고 처리 기능 제공
* 관리자 대시보드를 통한 운영 현황 통계 제공
* 프로필 이미지 업로드를 통한 사용자 개성 표현 지원
...
- CKEditor5 기반 이미지 업로드 지원
- 이미지는 서버의 `/uploads/posts` 경로에 저장
- 게시글 본문에는 `/uploads/posts/{filename}` 형태의 상대경로 저장
- Apache에서 `/uploads/**` 요청을 Spring Boot로 프록시 처리
- 이미지 확장자 및 Content-Type 검증
- 최대 업로드 크기 제한 적용

### HTML Sanitizing

게시글 본문은 CKEditor HTML을 저장하기 전에 jsoup 기반 Sanitizer를 거친다.

- `<script>`, `onerror`, `javascript:` 등 위험 요소 제거
- CKEditor 이미지 구조인 `<figure>`, `<img>` 허용
- `/uploads/posts/...` 형태의 상대경로 이미지 URL 허용
- 이미지 정렬/크기 유지를 위한 class, width, height 속성 허용
> 운영 환경에서는 이미지 URL을 절대경로가 아닌 상대경로(`/uploads/posts/...`)로 저장한다.
> 이를 위해 jsoup Safelist에서 이미지 `src`에 상대경로 프로토콜(`#`)을 허용하고 `preserveRelativeLinks(true)`를 사용한다.
...
![관리자 신고 관리](docs/images/admin-reports.png)
...
* 프로필 이미지 업로드 및 변경이 정상적으로 동작하는지 확인
* 게시글 상세에서 작성자 프로필 이미지가 정상적으로 표시되는지 확인
* 댓글에서 작성자 프로필 이미지가 정상적으로 표시되는지 확인
* 게시글 신고가 정상적으로 동작하는지 확인
* 댓글 신고가 정상적으로 동작하는지 확인
* 동일 대상 중복 신고가 차단되는지 확인
...
* 관리자 대시보드 통계가 정상적으로 조회되는지 확인
* 신고 목록 조회가 정상적으로 동작하는지 확인
* 신고 상태 및 대상 필터가 정상적으로 동작하는지 확인
* 신고 처리 완료 시 대기 중 신고 수가 감소하는지 확인
...
Apache2
  ├── /              → React dist
  ├── /posts/**      → React Router fallback
  ├── /api/**        → Spring Boot 8096 proxy
  └── /uploads/**    → Spring Boot 8096 proxy
```

## 운영 환경변수

운영 서버에서는 프로젝트 간 환경변수 충돌을 방지하기 위해 `FANFLOW_` prefix를 사용한다.

```env
SPRING_PROFILES_ACTIVE=prod
FANFLOW_SERVER_PORT=8096
FANFLOW_DB_URL=jdbc:mysql://localhost:8625/fanflow?serverTimezone=Asia/Seoul&characterEncoding=UTF-8
FANFLOW_DB_USERNAME=fanflow_user
FANFLOW_DB_PASSWORD=your_password
FANFLOW_JWT_SECRET=your_jwt_secret
FANFLOW_JWT_EXPIRATION=1800000
FANFLOW_FILE_UPLOAD_DIR=/home/ubuntu/fanflow/uploads
FANFLOW_CORS_ALLOWED_ORIGINS=https://fanflow.hongtfolio.kr
```
...
Spring Boot 애플리케이션은 JAR로 빌드 후 systemd 서비스로 실행한다.

```bash
sudo systemctl start fanflow
sudo systemctl stop fanflow
sudo systemctl restart fanflow
sudo systemctl status fanflow
```

### Frontend

React 애플리케이션은 Vite로 빌드한 뒤 Apache DocumentRoot에서 정적 파일로 제공한다.
...
- 운영 DB에서는 `ddl-auto=update`를 지속적으로 사용하지 않는다.
- 초기 테이블 생성 후에는 `validate`로 변경한다.
- 테스트용 sanitize 로그는 운영 배포 전에 제거한다.
- 업로드 파일 경로는 Git에 포함하지 않는다.
- 실제 `.env`, 서버 환경변수, JWT Secret, DB Password는 저장소에 커밋하지 않는다.
- 운영 환경에서는 Swagger UI와 API Docs를 비활성화한다.

## 향후 개선 사항

* 동영상 업로드 또는 외부 영상 임베드 기능 검토
* 관리자 대시보드 통계 세분화 및 기간별 차트 기능 추가
* 테스트 코드 보강
* 운영 로그 및 모니터링 개선
* Flyway 기반 DB migration 관리 도입 검토
